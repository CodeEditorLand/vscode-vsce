...
function assertProperty(
	...
	const property =
		manifest.PackageManifest.Metadata[0].Properties[0].Property.filter(
			(p) => p.$.Id === name
		);
...
describe("collect", function () {
	...
	it("should ignore devDependencies", () => {
		...
		return readManifest(cwd)
			...
			.then((files) => {
				...
				assert.ok(
					files.some((f) => /real\/dependency\.js/.test(f.path))
				);
				assert.ok(
					!files.some((f) => /fake\/dependency\.js/.test(f.path))
				);
			});
	});
	...
	it("should detect yarn", () => {
		...
		return readManifest(cwd)
			.then((manifest) =>
				collect(manifest, { cwd, dependencyEntryPoints: ["isexe"] })
			)
			.then((files) => {
				let seenWhich: boolean = false;
				let seenIsexe: boolean = false;
	...
	it("should skip all node_modules when dependencyEntryPoints is []", () => {
		...
		return readManifest(cwd)
			...
			.then((files) => {
				files.forEach((file) =>
					assert.ok(
						file.path.indexOf("/node_modules/which/") < 0,
						file.path
					)
				);
			});
...
describe("readManifest", () => {
	it("should patch NLS", async function () {
		...
		const translations = jsonc.parse(
			await fs.promises.readFile(
				path.join(cwd, "package.nls.json"),
				"utf8"
			)
		);
		const manifest = await readManifest(cwd);
...
describe("validateManifest", () => {
	...
	it("should validate activationEvents against main and browser", () => {
		assert.throws(() =>
			validateManifest(createManifest({ activationEvents: ["any"] }))
		);
		assert.throws(() =>
			validateManifest(createManifest({ main: "main.js" }))
		);
		assert.throws(() =>
			validateManifest(createManifest({ browser: "browser.js" }))
		);
		assert.throws(() =>
			validateManifest(
				createManifest({ main: "main.js", browser: "browser.js" })
			)
		);
		validateManifest(
			createManifest({ activationEvents: ["any"], main: "main.js" })
		);
		validateManifest(
			createManifest({ activationEvents: ["any"], browser: "browser.js" })
		);
		validateManifest(
			createManifest({
	...
	it("should validate extensionKind", () => {
		assert.throws(() =>
			validateManifest(createManifest({ extensionKind: ["web"] }))
		);
		assert.throws(() =>
			validateManifest(createManifest({ extensionKind: "web" }))
		);
		assert.throws(() =>
			validateManifest(
				createManifest({ extensionKind: ["workspace", "ui", "web"] })
			)
		);
		assert.throws(() =>
			validateManifest(
				createManifest({ extensionKind: ["workspace", "web"] })
			)
		);
		assert.throws(() =>
			validateManifest(createManifest({ extensionKind: ["ui", "web"] }))
		);
		assert.throws(() =>
			validateManifest(createManifest(<any>{ extensionKind: ["any"] }))
		);
		...
		validateManifest(createManifest({ extensionKind: ["workspace"] }));
		validateManifest(
			createManifest({ extensionKind: ["ui", "workspace"] })
		);
		validateManifest(
			createManifest({ extensionKind: ["workspace", "ui"] })
		);
	});

	it("should validate sponsor", () => {
		assert.throws(() =>
			validateManifest(createManifest({ sponsor: { url: "hello" } }))
		);
		assert.throws(() =>
			validateManifest(
				createManifest({ sponsor: { url: "www.foo.com" } })
			)
		);
		validateManifest(
			createManifest({ sponsor: { url: "https://foo.bar" } })
		);
		validateManifest(
			createManifest({ sponsor: { url: "http://www.foo.com" } })
		);
	});

	it("should validate pricing", () => {
		assert.throws(() =>
			validateManifest(createManifest({ pricing: "Paid" }))
		);
...
describe("toVsixManifest", () => {
	it("should produce a good xml", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) => {
				...
				assert.deepEqual(
					result.PackageManifest.Metadata[0].GalleryFlags,
					["Public"]
				);
				assert.strictEqual(
					result.PackageManifest.Installation.length,
					1
				);
				assert.strictEqual(
					result.PackageManifest.Installation[0].InstallationTarget
						.length,
					1
				);
				assert.strictEqual(
					result.PackageManifest.Installation[0].InstallationTarget[0]
						.$.Id,
				...
				assert.strictEqual(
					result.PackageManifest.Assets[0].Asset.length,
					1
				);
				assert.strictEqual(
	...
	it("should add a license metadata tag", () => {
		...
		return _toVsixManifest(manifest, files)
			...
			.then((result) => {
				...
				assert.strictEqual(
					result.PackageManifest.Metadata[0].License.length,
					1
				);
				assert.strictEqual(
	...
	it("should add an icon metadata tag", () => {
		...
		return _toVsixManifest(manifest, files)
			...
			.then((result) => {
				...
				assert.strictEqual(
					result.PackageManifest.Metadata[0].Icon.length,
					1
				);
				assert.strictEqual(
	...
	it("should understand gallery color and theme", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) => {
				const properties =
					result.PackageManifest.Metadata[0].Properties[0].Property.map(
						(p) => p.$
					);
				assert.ok(
					properties.some(
				...
				assert.ok(
					properties.some(
						(p) =>
							...
							p.Value ===
								"https://server.com/Microsoft/vscode-spell-check.git"
					)
				);
				assert.ok(
					properties.some(
				...
				assert.ok(
					properties.some(
						(p) =>
							...
							p.Value ===
								"https://server.com/Microsoft/vscode-spell-check"
					)
				);
			});
	});
					...
					properties.some(
						(p) =>
							p.Id ===
								"Microsoft.VisualStudio.Services.Links.Repository" &&
							p.Value ===
								"https://gitlab.com/Microsoft/vscode-spell-check.git"
					)
				);
				assert.ok(
					properties.some(
						(p) =>
							p.Id ===
								"Microsoft.VisualStudio.Services.Links.Support" &&
							p.Value ===
								"https://gitlab.com/Microsoft/vscode-spell-check/issues"
					)
				);
				assert.ok(
					properties.some(
						(p) =>
							p.Id ===
								"Microsoft.VisualStudio.Services.Links.Learn" &&
							p.Value ===
								"https://gitlab.com/Microsoft/vscode-spell-check#readme"
					)
				);
			});
	});
	...
	it("should understand categories", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) => {
				const categories =
					result.PackageManifest.Metadata[0].Categories[0].split(",");
	...
	it("should not automatically add theme tag when themes are empty", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) =>
				assert.deepEqual(
					result.PackageManifest.Metadata[0].Tags[0],
					"__web_extension"
				)
			);
	});
	...
	it("should detect keywords in description", () => {
		const manifest = {
			...
			engines: Object.create(null),
			description:
				"This C++ extension likes combines ftp with javascript",
		};
	...
	it("should expose localization contributions as assets", () => {
		...
		const files = [
			...
			{
				path: "extension/translations/pt.json",
			...
			.then((result) => {
				...
				assert.ok(
					assets.some(
						(asset) =>
							asset.$.Type ===
								"Microsoft.VisualStudio.Code.Translation.PT" &&
	...
	it("should understand badges", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) => {
				const badges =
					result.PackageManifest.Metadata[0].Badges[0].Badge;
				...
				assert.strictEqual(
					badges[1].$.ImgUri,
					"http://anotherbadgeurl.png"
				);
				assert.strictEqual(
					badges[1].$.Description,
					"this is another badge"
				);
			});
	});
	...
	it("should not have empty keywords #114", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) => {
				...
				tags.forEach((tag) =>
					assert.ok(tag, `Found empty tag '${tag}'.`)
				);
			});
	});
	...
	it("should expose extension kind properties when provided", async () => {
		...
		const properties =
			result.PackageManifest.Metadata[0].Properties[0].Property;
		const extensionKindProps = properties.filter(
			(p) => p.$.Id === "Microsoft.VisualStudio.Code.ExtensionKind"
		);
		assert.strictEqual(
			extensionKindProps[0].$.Value,
			["ui", "workspace", "web"].join(",")
		);
	});
	...
	it("should not have target platform by default", async () => {
		...
		assert.strictEqual(
			dom.PackageManifest.Metadata[0].Identity[0].$.Id,
			"test"
		);
		assert.strictEqual(
			dom.PackageManifest.Metadata[0].Identity[0].$.Version,
			"0.0.1"
		);
		assert.strictEqual(
			dom.PackageManifest.Metadata[0].Identity[0].$.Publisher,
			"mocha"
		);
		assert.strictEqual(
			dom.PackageManifest.Metadata[0].Identity[0].$.TargetPlatform,
			undefined
		);
	});
	...
	it("should fail when target is invalid", async () => {
		...
		try {
			...
			return assert.ok(
				/is not a valid VS Code target/i.test(err.message)
			);
		}
	...
	it("should add sponsor link property", () => {
		...
		return _toVsixManifest(manifest, [])
			...
			.then((result) => {
				const properties =
					result.PackageManifest.Metadata[0].Properties[0].Property;
				const sponsorLinkProp = properties.find(
					(p) => p.$.Id === "Microsoft.VisualStudio.Code.SponsorLink"
				);
	...
	it("should include extra extensions", () => {
		...
		return toContentTypes(files)
			...
			.then((result) => {
				...
				assert.ok(
					!result.Types.Default.some((d) => d.$.Extension === "")
				);
			});
	});
});
...
describe("LaunchEntryPointProcessor", () => {
	it("should detect when declared entrypoint is not in package", async () => {
		...
		try {
			...
			didErr =
				message.includes("entrypoint(s) missing") &&
	...
	it("should work even if .js extension is not used", async () => {
		...
		const files = [
			{
				path: "extension/out/src/extension.js",
				contents: Buffer.from(""),
			},
		];
		await _toVsixManifest(manifest, files);
	...
	it("should accept manifest if no entrypoints defined", async () => {
		const manifest = createManifest({});
		const files = [
			{ path: "extension/something.js", contents: Buffer.from("") },
		];
		await _toVsixManifest(manifest, files);
	});
});
...
describe("MarkdownProcessor", () => {
	...
	it("should take baseContentUrl", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(path.join(root, "readme.expected.md"), "utf8")
					.then((expected) => {
	...
	it("should infer baseContentUrl if its a github repo", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(path.join(root, "readme.default.md"), "utf8")
					.then((expected) => {
						assert.strictEqual(actual, expected);
					});
			});
	...
	it("should replace relative links with GitHub URLs while respecting githubBranch", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(
						path.join(root, "readme.branch.main.expected.md"),
						"utf8"
					)
					.then((expected) => {
						assert.strictEqual(actual, expected);
	...
	it("should infer baseContentUrl if its a gitlab repo", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(
						path.join(root, "readme.gitlab.default.md"),
						"utf8"
					)
					.then((expected) => {
						assert.strictEqual(actual, expected);
	...
	it("should not replace issue links with urls if its a github repo but issue link expansion is disabled.", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(path.join(root, "readme.github.md"), "utf8")
					.then((expected) => {
						assert.strictEqual(actual, expected);
					});
			});
	...
	it("should not replace issue links with urls if its not a github repo.", () => {
		const manifest = {
			...
			engines: Object.create(null),
			repository:
				"https://some-other-provider.com/username/repository.git",
		};
	...
	it("should replace issue links with urls if its a gitlab repo.", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(
						path.join(root, "readme.gitlab.expected.md"),
						"utf8"
					)
					.then((expected) => {
						assert.strictEqual(actual, expected);
	...
	it("should not replace issue links with urls if its a gitlab repo but issue link expansion is disabled.", () => {
		...
		return processor
			...
			.then((actual) => {
				return fs.promises
					.readFile(path.join(root, "readme.gitlab.md"), "utf8")
					.then((expected) => {
						assert.strictEqual(actual, expected);
					});
			});
...
describe("version", function () {
	...
	beforeEach(() => {
		...
		cwd = dir.name;
		fs.copyFileSync(
			path.join(fixtureFolder, "package.json"),
