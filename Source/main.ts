...
module.exports = function (argv: string[]): void {
	...
	program
		...
		.option(
			"--no-yarn",
			"Use npm instead of yarn (default inferred from absence of yarn.lock or .yarnrc)",
		)
		.option<string[]>(
		...
		.option(
			"--dependencies",
			"Enable dependency detection via npm or yarn",
			undefined,
		)
		.option(
			"--no-dependencies",
			"Disable dependency detection via npm or yarn",
			undefined,
		)
		.option(
			"--readme-path <path>",
	...
	program
		...
		.option(
			"-t, --target <target>",
			`Target architecture. Valid targets: ${ValidTargets}`,
		)
		.option(
			"--ignore-other-target-folders",
			`Ignore other target folders. Valid only when --target <target> is provided.`,
		)
		.option(
			"--readme-path <path>",
			"Path to README file (defaults to README.md)",
		...
		.option(
			...
			"Allow include patterns for the files field in package.json that does not match any file",
		)
		.option("--skip-license", "Allow packaging without license file")
		.option(
			"--sign-tool <path>",
			"Path to the VSIX signing tool. Will be invoked with two arguments: `SIGNTOOL <path/to/extension.signature.manifest> <path/to/extension.signature.p7s>`.",
		)
		.option(
			"--follow-symlinks",
			"Recurse into symlinked directories instead of treating them as files",
	...
	program
		...
		.option(
			"-t, --target <targets...>",
			`Target architectures. Valid targets: ${ValidTargets}`,
		)
		.option(
			"--ignore-other-target-folders",
			`Ignore other target folders. Valid only when --target <target> is provided.`,
		...
		.option(
			"-i, --packagePath <paths...>",
			"Publish the provided VSIX packages.",
		)
		.option(
			"--manifestPath <paths...>",
			"Manifest files to publish alongside the VSIX packages.",
		)
		.option(
			"--signaturePath <paths...>",
			"Signature files to publish alongside the VSIX packages.",
		)
		.option(
			"--sigzipPath <paths...>",
			"Signature archives to publish alongside the VSIX packages.",
		)
		.option(
			"--sign-tool <path>",
			"Path to the VSIX signing tool. Will be invoked with two arguments: `SIGNTOOL <path/to/extension.signature.manifest> <path/to/extension.signature.p7s>`. This will be ignored if --sigzipPath is provided.",
		)
		.option(
			"--githubBranch <branch>",
			"The GitHub branch used to infer relative links in README.md. Can be overridden by --baseContentUrl and --baseImagesUrl.",
		...
		.option(
			"--no-verify",
			"Allow all proposed APIs (deprecated: use --allow-all-proposed-apis instead)",
		)
		.addOption(
			new Option(
				"--noVerify",
		...
		.option(
			"--allow-proposed-apis <apis...>",
			"Allow specific proposed APIs",
		)
	...
	program
		...
		.description(
			"Unpublishes an extension. Example extension id: ms-vscode.live-server.",
		)
		...
		.option(
			"-f, --force",
			"Skip confirmation prompt when unpublishing an extension",
		)
		.action((id, { pat, azureCredential, force }) =>
			main(unpublish({ id, pat, azureCredential, force })),
		);

	program
		.command("generate-manifest")
		.description(
			"Generates the extension manifest from the provided VSIX package.",
		)
		.requiredOption("-i, --packagePath <path>", "Path to the VSIX package")
		.option(
			"-o, --out <path>",
			"Output the extension manifest to <path> location (defaults to <packagename>.manifest)",
		)
		.action(({ packagePath, out }) =>
			main(generateManifest(packagePath, out)),
		);

	program
		.command("verify-signature")
		.description(
			"Verifies the provided signature file against the provided VSIX package and manifest.",
		)
		.requiredOption("-i, --packagePath <path>", "Path to the VSIX package")
		.requiredOption(
			"-m, --manifestPath <path>",
	...
	program
		...
		.description(
			"Verifies if the Personal Access Token or Azure identity has publish rights for the publisher",
		)
		.option(
			"-p, --pat <token>",
			"Personal Access Token (defaults to VSCE_PAT environment variable)",
		...
		.action((publisherName, { pat, azureCredential }) =>
			main(verifyPat({ publisherName, pat, azureCredential })),
		);
	...
	program
		...
		.action((text, { json, pagesize, stats }) =>
			main(search(text, json, parseInt(pagesize), stats)),
		);
