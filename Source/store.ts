...
export class FileStore implements IStore {
	...
	static async open(
		path: string = FileStore.DefaultPath,
	): Promise<FileStore> {
		try {
	...
	private async save(): Promise<void> {
		await fs.promises.writeFile(
			this.path,
			JSON.stringify({ publishers: this.publishers }),
			{ mode: "0600" },
		);
	}
...
export async function verifyPat(options: IVerifyPatOptions): Promise<void> {
	...
	console.log(
		`The Personal Access Token verification succeeded for the publisher '${publisherName}'.`,
	);
}
